...
import { KnowledgeItem, KnowledgeCategory, KnowledgeDifficulty } from '../../types/knowledge';
import {
  BookOpen,
  Search,
  Bookmark,
...
const DIFFICULTY_CONFIG: Record<KnowledgeDifficulty, { label: string; color: string }> = {
  foundation: { label: '基础通用', color: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
  advanced: { label: '进阶实战', color: 'bg-blue-50 text-blue-700 border-blue-200' },
  big_tech_must: { label: '大厂必考', color: 'bg-amber-50 text-amber-800 border-amber-200' },
  architecture: { label: '架构深度', color: 'bg-purple-50 text-purple-700 border-purple-200' },
};

export const KnowledgeBase: React.FC<KnowledgeBaseProps> = ({ items, onUpdateItems }) => {
  ...
  return (
    <div className="space-y-4">
      {/* Top Banner */}
      <div className="bg-gradient-to-r from-slate-900 via-blue-950 to-slate-900 text-white p-5 sm:p-6 rounded-2xl border border-blue-900/60 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase bg-blue-500 text-white">
              Tech Interview Knowledge Base
            </span>
            <span className="text-xs text-blue-200">大厂高频考点 · 原理剖析 · 高分答题模板</span>
          </div>
          <h2 className="text-xl font-black tracking-tight">程序员技术面试基础知识库</h2>
          <p className="text-xs text-blue-200 mt-1 max-w-2xl leading-relaxed">
            涵盖前端现代工程、高并发分布式后端、高频算法模型、系统设计、大模型 AI 全栈及 STAR 表达体系，助您在复盘中查漏补缺、举一反三！
          </p>
        </div>
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-xs font-bold shadow-md shadow-blue-950/40 transition-colors flex-shrink-0"
        >
            ...
            >
              ...
              >
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  >
                    <Bookmark className={`w-4 h-4 ${item.isBookmarked ? 'fill-amber-500 text-amber-500' : ''}`} />
                  </button>
                    ...
                    <div className="bg-amber-50/50 p-4 rounded-xl border border-amber-200/80 space-y-2">
                      ...
                      <div className="space-y-1.5">
                        {item.interviewerQuestions.map((iq, idx) => (
                          <div key={idx} className="p-2 bg-white/90 rounded-lg border border-amber-100 font-medium text-amber-950">
                            Q{idx + 1}: {iq}
                          </div>
                        ))}
                      </div>
