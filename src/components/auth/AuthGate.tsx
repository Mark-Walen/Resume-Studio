...
function LoadingScreen() {
  return (
    <div className="min-h-screen bg-[#f5f5f7] flex items-center justify-center text-slate-600">
...
function AuthDialog({ mode: initialMode, onClose }: { mode: AuthMode; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm" role="dialog" aria-modal="true">
      <div className="w-full max-w-md overflow-hidden rounded-3xl border border-white/50 bg-white shadow-2xl">
        <div className="flex items-start justify-between border-b border-slate-100 px-6 py-5">
          ...
          <button onClick={onClose} className="rounded-xl p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700" aria-label="关闭">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-6">
          {mode !== 'reset' && (
            <button
              ...
              className="flex w-full items-center justify-center gap-3 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-50"
            >
          ...
          {mode === 'register' && (
            <label className="block text-xs font-bold text-slate-700">
              昵称
              <input value={name} onChange={event => setName(event.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-300 px-3.5 py-2.5 font-normal outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100" placeholder="如何称呼你" autoComplete="name" />
            </label>
          )}

          <label className="block text-xs font-bold text-slate-700">
            邮箱
            <input required type="email" value={email} onChange={event => setEmail(event.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-300 px-3.5 py-2.5 font-normal outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100" placeholder="name@example.com" autoComplete="email" />
          </label>

          {mode !== 'reset' && (
            <label className="block text-xs font-bold text-slate-700">
              密码
              <input required minLength={6} type="password" value={password} onChange={event => setPassword(event.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-300 px-3.5 py-2.5 font-normal outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100" placeholder={mode === 'register' ? '至少 10 位，建议包含大小写字母和数字' : '输入密码'} autoComplete={mode === 'register' ? 'new-password' : 'current-password'} />
            </label>
          )}
...
function GuestLanding() {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#f5f5f7] text-slate-950">
      <header className="border-b border-black/5 bg-white/85 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-8">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setAuthMode('signin')} className="rounded-xl px-4 py-2 text-sm font-bold text-slate-600 hover:bg-slate-100">登录</button>
        ...
        <section className="relative mx-auto grid max-w-7xl items-center gap-12 px-5 py-16 sm:px-8 lg:grid-cols-[1.05fr_0.95fr] lg:py-24">
          <div className="relative z-10">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <button onClick={() => setAuthMode('signin')} className="rounded-2xl border border-slate-300 bg-white px-5 py-3 text-sm font-bold text-slate-700 hover:border-slate-400">已有账户，直接登录</button>
            </div>
          ...
          <div className="relative">
            ...
            <div className="relative rounded-[2rem] border border-white/70 bg-white/90 p-5 shadow-2xl shadow-slate-300/45 backdrop-blur">
              ...
              <div className="mt-4 space-y-3">
                {['嵌入式软件工程师 · 技术二面', 'IoT 平台开发 · 简历精修', '系统工程师 · 公司背调'].map((item, index) => (
                  <div key={item} className="flex items-center gap-3 rounded-2xl border border-slate-100 p-3.5">
...
function VerifyEmailScreen() {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f7] p-5 flex items-center justify-center">
      <div className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-7 text-center shadow-xl">
