...
export const ResumeEditor: React.FC<ResumeEditorProps> = ({ resume, onChange, onOpenAiGenerator }) => {
  ...
  const renderCertificates = () => draft && <div className="space-y-3">{draft.certificates.map(item => <div key={item.id} className="grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 sm:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)_9rem_auto]"><Field label="证书或荣誉名称"><input className={inputClass} value={item.name} onChange={e => updateDraft({ certificates: draft.certificates.map(value => value.id === item.id ? { ...value, name: e.target.value } : value) })} placeholder="填写证书或荣誉名称" /></Field><Field label="颁发机构"><input className={inputClass} value={item.issuer} onChange={e => updateDraft({ certificates: draft.certificates.map(value => value.id === item.id ? { ...value, issuer: e.target.value } : value) })} placeholder="选填" /></Field><Field label="获得时间"><input type="month" className={inputClass} value={item.date || ''} onChange={e => updateDraft({ certificates: draft.certificates.map(value => value.id === item.id ? { ...value, date: e.target.value } : value) })} /></Field><button type="button" aria-label={`删除${item.name}`} onClick={() => updateDraft({ certificates: draft.certificates.filter(value => value.id !== item.id) })} className="mt-6 rounded-lg p-2 text-slate-400 hover:bg-red-50 hover:text-red-600"><Trash2 className="h-4 w-4" /></button></div>)}<button onClick={() => updateDraft({ certificates: [...draft.certificates, { id: `cert-${Date.now()}`, name: '新证书', issuer: '', date: '' }] })} className="inline-flex items-center gap-1 text-xs font-semibold text-blue-600"><Plus className="h-4 w-4" />添加证书或荣誉</button></div>;
