...
export const WorkDailyLogDashboard: React.FC<WorkDailyLogDashboardProps> = ({
  ...
  return (
    <div id="work-daily-log-dashboard" className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/80 p-6 rounded-2xl border border-slate-800">
        ...
        <div className="flex items-center space-x-2.5">
          <button
            ...
            className="flex items-center space-x-1.5 px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold rounded-xl border border-slate-700 transition-all"
          >
          ...
          <button
            ...
            className="flex items-center space-x-1.5 px-5 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 disabled:opacity-50 text-white text-xs font-bold rounded-xl shadow-lg shadow-emerald-600/20 transition-all"
          >
      ...
      {extractResult && (
        <div className="p-6 rounded-2xl bg-gradient-to-br from-emerald-950/40 via-slate-900 to-slate-900 border border-emerald-500/40 shadow-xl space-y-4">
          <div className="flex items-center justify-between border-b border-emerald-500/20 pb-3">
            <div className="flex items-center space-x-2">
              <Sparkles className="w-5 h-5 text-emerald-400" />
              <h3 className="text-base font-bold text-white">AI 智能提炼生成的专业简历亮点</h3>
            </div>
          ...
          <p className="text-xs text-slate-300 leading-relaxed">
            <span className="text-emerald-400 font-semibold">能力画像归纳：</span>
          ...
          <div className="space-y-3">
            {extractResult.suggestedBullets.map((b) => {
              ...
              return (
                <div
                  key={b.id}
                  className="p-4 rounded-xl bg-slate-900/80 border border-emerald-500/30 space-y-2.5"
                >
                  ...
                  <p className="text-xs text-emerald-200/90 font-mono leading-relaxed bg-emerald-950/20 p-3 rounded-lg border border-emerald-500/20">
                    {b.bulletText}
                  </p>
                  ...
                  {b.starBreakdown && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] text-slate-400 pt-1">
                      <div>
                        <span className="text-indigo-400 font-medium">[S/T 业务痛点]：</span>
                        {b.starBreakdown.situation}
                      </div>
                      <div>
                        <span className="text-emerald-400 font-medium">[A/R 方案与成效]：</span>
      ...
      <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 space-y-4">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          ...
          <span className="text-xs text-slate-400">已勾选 {selectedLogIds.size} 条用于提炼</span>
        </div>
            ...
            return (
              ...
              >
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center space-x-2">
                        ...
                        <h4 className="text-sm font-bold text-white">{log.projectOrModuleName}</h4>
                      </div>
                ...
                <div className="mt-3 pl-7 space-y-2 text-xs">
                  <div className="p-3 rounded-lg bg-slate-900/60 border border-slate-700/60 text-slate-300 leading-relaxed">
                    <span className="text-amber-400 font-semibold">【技术攻坚与解法】：</span>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
                    ...
                    {log.evidences && log.evidences.length > 0 && (
                      <div className="flex items-center space-x-1.5 text-[11px] text-indigo-400">
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleSaveLog} className="p-6 space-y-4 overflow-y-auto flex-1">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1">记录日期</label>
                  <input
                    type="date"
                    required
                    value={formDate}
                ...
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1">所属分类</label>
                  <select
                    value={formCategory}
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">今日完成任务概要</label>
                <textarea
                  rows={2}
                  required
              ...
              <div className="p-3 rounded-xl bg-slate-800/60 border border-slate-700/60 space-y-2">
                <div className="text-xs font-medium text-slate-200">经历凭证绑定 (PR / 压测报告 / 架构文档)：</div>
