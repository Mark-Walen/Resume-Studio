      ...
      <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-4xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden text-slate-900">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-slate-50">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900">基于目标公司 JD 智能推荐知识考点</h3>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-700 mb-1">即将面试的职位</label>
                <input
                  type="text"
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3 py-2 bg-slate-100 border border-slate-200 rounded-lg text-xs text-slate-900 focus:outline-none focus:border-indigo-500 resize-none"
              />
            </div>
            ...
            <div className="flex justify-end pt-1">
              <button
                ...
                disabled={isLoading}
                className="flex items-center space-x-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-xs font-semibold rounded-xl shadow-sm transition-all"
              >
          ...
          {errorMsg && (
            <div className="p-3 bg-red-500/10 border border-red-500/30 rounded-xl text-xs text-red-700 flex items-center space-x-2">
                  ...
                  <div className="flex items-center space-x-2.5">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-blue-700 font-mono">
                        对齐匹配度评分：<span className="font-bold text-amber-600">{result.overallMatchScore}分</span>
                      </div>
                    </div>
                  </div>
                  <span className="px-2.5 py-1 rounded-full text-xs font-medium bg-amber-500/10 text-amber-700 border border-amber-500/20">
                    针对性技术突击
                  </span>
                </div>

                <div className="text-xs text-slate-700 leading-relaxed">
                  <span className="text-blue-600 font-semibold">【企业技术风格画像】：</span>
                  {result.companyTechProfile}
                </div>

                {result.coreRequirementsSummary && (
                  <div className="text-xs text-slate-500 leading-relaxed">
                    <span className="text-slate-700 font-medium">【核心硬性门槛】：</span>
                    {result.coreRequirementsSummary}
                  </div>
                )}
              </div>
                ...
                <div className="space-y-3">
                  {result.recommendations.map((rec) => {
                    ...
                    return (
                      <div
                        key={rec.id}
                        className="p-4 rounded-xl bg-slate-100/70 border border-slate-200 hover:border-indigo-500/40 space-y-3 transition-colors"
                      >
                              ...
                              <button
                                ...
                                className={`flex items-center space-x-1 px-2.5 py-1 text-xs rounded-lg transition-colors ${
                                  isSaved
                                    ...
                                    : 'bg-indigo-600/20 hover:bg-indigo-600/30 text-blue-700 border border-indigo-500/30'
                                }`}
                              >
                                {isSaved ? (
                                  <>
                        ...
                        {/* Match Reason */}
                        <div className="text-xs text-slate-700 leading-relaxed">
                          <span className="text-amber-600 font-medium">为什么必考：</span>
                          {rec.matchReason}
                        </div>

                        {/* Specific flavor */}
                        <div className="p-2.5 rounded-lg bg-blue-50 border border-indigo-500/20 text-xs text-blue-700 space-y-1">
                          <div className="font-semibold text-blue-700 flex items-center space-x-1.5">
                        ...
                        {rec.interviewTrapWarning && (
                          <div className="p-2.5 rounded-lg bg-red-50 border border-red-500/20 text-xs text-red-700 flex items-start space-x-2">
                            <ShieldAlert className="w-4 h-4 text-red-600 flex-shrink-0 mt-0.5" />
                            <div>
                              <span className="font-semibold text-red-700">易踩坑翻车点：</span>
                              <span className="ml-1 text-red-700/90">{rec.interviewTrapWarning}</span>
                            </div>
                          </div>
                        )}

                        {/* Preparation Action */}
                        <div className="text-xs text-emerald-700/90 bg-emerald-50 p-2.5 rounded-lg border border-emerald-500/20">
                          <span className="font-semibold text-emerald-700 inline-flex items-center gap-1.5"><Lightbulb className="w-3.5 h-3.5" />面试标准回答思路：</span>
