    ...
    <div className="space-y-4">
      ...
      <div className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-xs flex flex-col sm:flex-row justify-between items-center gap-3">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          {onOpenCalendar && (
            <button
              id="btn-open-calendar-from-pipeline"
              onClick={onOpenCalendar}
              className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 rounded-lg text-xs font-bold shadow-2xs transition-colors w-full sm:w-auto justify-center"
            >
                  ...
                  >
                    ...
                    <div className="space-y-1 my-2 text-[10px] text-slate-500">
                      ...
                      {job.scheduledInterviewDate && (
                        <div className="flex items-center gap-1 text-[10px] text-indigo-700 bg-indigo-50 border border-indigo-200 rounded px-1.5 py-0.5 mt-1 font-mono">
                          <Clock className="w-3 h-3 text-indigo-500 shrink-0" />
                          <span className="truncate">
                            面试: {new Date(job.scheduledInterviewDate).toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                          </span>
                        </div>
                      )}
                      {job.notes && (
                      ...
                      {/* Dossier & JD Recommender Shortcuts */}
                      <div className="flex items-center gap-1.5 pt-1.5 flex-wrap">
                        {onOpenDossier && (
                          <button
                            onClick={() => onOpenDossier(job)}
                            className="inline-flex items-center gap-1 text-[10px] font-medium text-slate-600 hover:text-indigo-600 bg-slate-50 hover:bg-indigo-50 border border-slate-200 hover:border-indigo-200 rounded px-1.5 py-0.5 transition-colors"
                            title="查看与补充公司HR简介、待遇、团队及背调资料合集"
                          >
                            <FileText className="w-2.5 h-2.5 text-indigo-500" />
                            <span>背调资料</span>
                            {job.companyDossier?.collectedLinks && job.companyDossier.collectedLinks.length > 0 && (
                              <span className="px-1 rounded-full bg-indigo-100 text-indigo-700 font-bold text-[9px]">
                                {job.companyDossier.collectedLinks.length}
                              </span>
                            )}
                          </button>
                        )}
                        ...
                        {onOpenJdRecommend && (
                          <button
                            ...
                            className="inline-flex items-center gap-1 text-[10px] font-medium text-amber-700 hover:text-amber-800 bg-amber-50 hover:bg-amber-100 border border-amber-200 rounded px-1.5 py-0.5 transition-colors"
                            title="根据此岗位 JD 推荐考点与大厂特色"
                          >
