        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-950/80">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <div className="flex items-center space-x-2">
                <h3 className="text-base font-bold text-white">{application.companyName}</h3>
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="flex items-center space-x-1.5 px-3 py-1.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 border border-amber-500/30 rounded-lg text-xs font-medium transition-colors"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="p-4 rounded-xl bg-slate-800/60 border border-slate-700/80 space-y-2">
              ...
              <textarea
                ...
                className="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-lg text-xs text-white focus:outline-none focus:border-indigo-500 resize-none"
              />
            </div>
          ...
          <div className="p-4 rounded-xl bg-slate-800/60 border border-slate-700/80 space-y-3">
            ...
            <div className="space-y-2">
              {dossier.reverseQuestions?.map((q, idx) => (
                <div
                  key={idx}
                  className="flex items-start justify-between p-2.5 bg-slate-900 rounded-lg text-xs border border-slate-700/60"
                >
                  <span className="text-slate-200 leading-relaxed pr-2">
                    {idx + 1}. {q}
                  </span>
                  <button
          ...
          <div className="p-4 rounded-xl bg-slate-800/60 border border-slate-700/80 space-y-3">
            ...
            <div className="space-y-2">
              {dossier.collectedLinks?.map((link) => (
                <div
                  key={link.id}
                  className="flex items-center justify-between p-3 bg-slate-900 rounded-lg text-xs border border-slate-700/60"
                >
                  <div className="space-y-0.5 min-w-0 pr-3">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="font-medium text-indigo-300 hover:text-indigo-200 flex items-center space-x-1 truncate"
                    >
                      <span className="truncate">{link.title}</span>
                      <ExternalLink className="w-3 h-3 flex-shrink-0" />
                    </a>
                    {link.note && <div className="text-[11px] text-slate-400 truncate">{link.note}</div>}
                  </div>
                  <button
              ...
              <div className="p-3 bg-slate-900/80 rounded-lg border border-slate-700/60 space-y-2">
                <div className="text-[11px] font-medium text-slate-300">添加新资料链接：</div>
