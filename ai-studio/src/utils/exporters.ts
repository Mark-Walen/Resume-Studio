...
export function exportToWord(resume: ResumeData): void {
  ...
  </style></head><body><div class="header">${avatar}<h1>${escapeHtml(resume.personalInfo.fullName)}</h1><div class="subtitle">${escapeHtml(resume.personalInfo.jobTitle)}</div><div class="contact">邮箱：${escapeHtml(resume.personalInfo.email)}　电话：${escapeHtml(resume.personalInfo.phone)}　地点：${escapeHtml(resume.personalInfo.location)}${resume.personalInfo.expectedSalary ? `　期望薪资：${escapeHtml(resume.personalInfo.expectedSalary)}` : ''}${resume.personalInfo.github ? `　GitHub：${escapeHtml(resume.personalInfo.github)}` : ''}</div></div>${getSectionOrder(resume).map(key => wordSection(resume, key)).join('')}</body></html>`;
...
export function exportToMarkdown(resume: ResumeData): void {
  ...
  if (resume.personalInfo.expectedSalary) header.push(`- **期望薪资**：${resume.personalInfo.expectedSalary}`);
